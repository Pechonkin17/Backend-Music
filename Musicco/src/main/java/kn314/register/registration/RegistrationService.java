...
import kn314.register.email.EmailSender;
...
import kn314.register.validation.PasswordValidator;
...
public class RegistrationService {
    ...
    public String register(RegistrationRequest registrationRequest){
        ...
        if (!isValidPassword){
            throw new ResponseStatusException(
                    HttpStatus.BAD_REQUEST, "The password must contain uppercase letters and numbers."
            );
        }

        if (userService.existsByEmail(registrationRequest.getEmail())) {
            User user = userService.findByEmail(registrationRequest.getEmail());
        ...
        String link = "http://localhost:8080/Musicco/registration/confirm?token=" + token;
        emailSender.send(
                registrationRequest.getEmail(),
                buildEmail(registrationRequest.getUsername(),
                        link
                ));
        ...
        ConfirmationToken confirmationToken = confirmationTokenService
                ...
                .orElseThrow(() ->
                        new ResponseStatusException(
                                HttpStatus.BAD_REQUEST, "Token not found"));
